from pyhcl import *
from vta.util import * #util少文件
from vta.shell import *

class Compute(Module)(debug: Boolean = false)(implicit p: Parameters) {  //?
    def __init__(self):
        mp = p(ShellKey).memParams #?
        io = IO(
            i_post = Vec(2,  Input(Bool))
            o_post = Vec(2, Output(Bool))
            #inst =
            #uop_baddr = Input(Uint()) 
            #acc
