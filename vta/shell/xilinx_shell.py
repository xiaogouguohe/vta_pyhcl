...
class XilinxShellGmem_IO:
    def __init__(self):
        self.xilinxAXIMaster = XilinxAXIMaster()

class XilinxShellCtrl_IO(Bundle_Helper):
    def __init__(self):
        self.xilinxAXILiteClient = XilinxAXILiteClient()

class XilinxShell(Module): #extends RawModule 

    p = ShellKey()
    hp = p.hostParams
    mp = p.memParams

    ap_clk = IO(a = Input(Clock))
    ap_rst_n = IO(a = Input(Bool))
    m_axi_gmem = mapper(XilinxShellGmem_IO())
    s_axi_control = mapper(XilinxShellCtrl_IO())
    m_axi_gmem = mapper(XilinxAXIMaster())
    s_axi_control = mapper(XilinxAXILiteClient())
...
if __name__ == '__main__':
    Emitter.dumpVerilog(Emitter.dump(Emitter.emit(XilinxShell()), "XilinxShell.fir"))
